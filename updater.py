...
"""
IGUV Weekly Updater – HTML only (mit Websuche über Responses API + tools=[{"type":"web_search"}])

- Modell: gpt-5  (kein Temperature-Parameter setzen; Default=1)
- Websuche: explizit per tools aktiviert (USE_WEBSEARCH=1)
- Struktur & Layout gemäss Vorgaben (Poppins, grössere Abstände, CH-Datum mit Uhrzeit)
- FINMA wird breit geprüft (inkl. Sanktionen & Embargos-Seite), EU inkl. sanctionsmap.eu
- Medien/Verbände & Parlament bis 60 Tage rückwärts, wenn relevant
- Events: nächste Termine von /event/ (n=3)
- Ausgabe via MU-Plugin-Endpoint /wp-json/iguv/v1/weekly (optional X-IGUV-Token)
"""
...
USER_AGENT = "Mozilla/5.0 (compatible; IGUV-Weekly-Updater/10.0; +https://iguv.ch)"
REQ_TIMEOUT = 30
...
<style>
  ...
  .iguv-weekly h1{font-size:2.2rem;margin:0 0 .8rem;color:#0f2a5a;font-weight:700}
  .iguv-weekly .meta{color:#666;margin:0 0 1.4rem}
...
def main():
    ...
    print("KI-Zusammenfassung (gpt-5 + Websuche) …")
